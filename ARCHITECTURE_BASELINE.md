...
| Command Center THÁNOS | Camada visual responsiva sobre chat/voz, workspace, evidence, joias, poderes e confirmação. USER recebe uma superfície conversacional simplificada; SUPERADMIN recebe uma visão de controle apenas quando o backend autoriza. | A UI não cria autoridade, não expõe grants, payloads, erros brutos, secrets ou allowlists. WhatsApp, Telegram, n8n, Hermes remoto e WRITE público continuam desativados; capacidades futuras aparecem como `Em breve` ou `Desativado`. |
